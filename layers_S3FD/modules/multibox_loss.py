...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        loss_l = F.smooth_l1_loss(loc_p.cuda(), loc_t.cuda(), size_average=False).cuda()
        # print(loc_p)
        # Compute max conf across batch for hard negative mining
        batch_conf = conf_data.view(-1, self.num_classes).cuda()
        ...
        loss_c = F.cross_entropy(conf_p.cuda(), targets_weighted.cuda(), size_average=False).cuda()
